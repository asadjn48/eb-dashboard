...
const EditEmployee = () => {
  ...
  // 1. THIS IS THE CRITICAL FIX: Check if the ID is 'new'
  // const isNew = id === 'new';
  const isNew = !id || id === 'new';

  // 2. If it's new, DO NOT start in a loading state!
  ...
  // Fetch Data (Only if editing an existing employee)
  useEffect(() => {
    // 3. IF IT IS A NEW EMPLOYEE, STOP HERE. DO NOT FETCH.
    if (isNew) {
      setIsLoading(false);
      return; 
    }
        ...
        <div className="flex items-center gap-2">
            ...
            <div>
                <h1 className="text-lg font-bold text-gray-900 leading-none">
                  {isNew ? 'Add New Member' : 'Edit Employee'}
                </h1>
                <p className="text-xs text-gray-500 mt-0.5">
                  {isNew ? 'Register a new employee.' : 'Update profile information.'}
                </p>
            </div>
        </div>
        <div className="flex gap-2">
            ...
            <Button className="bg-slate-900 text-white h-8 text-xs hover:bg-slate-800" size="sm" onClick={handleSave} disabled={isSaving}>
                {isSaving ? 'Saving...' : <><Save className="w-3.5 h-3.5 mr-1.5" /> Save Changes</>}
            </Button>
        </div>
      </div>
